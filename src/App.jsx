...
function parseJSON(raw){
  ...
  // Extract first {...} or [...] block from anywhere in the text
  const objMatch = stripped.match(/\{[\s\S]*\}/);
  if(objMatch) try { return JSON.parse(objMatch[0]); } catch{}
  const arrMatch = stripped.match(/\[[\s\S]*\]/);
  if(arrMatch) try { return JSON.parse(arrMatch[0]); } catch{}
  throw new Error("Réponse JSON invalide — réessaie ou vérifie les captures d'écran.");
}
...
function ruleBasedClean(kws){
  return kws.filter(({keyword})=>{
    const kw=keyword.toLowerCase();
    if(BRAND_BLACKLIST.some(b=>kw.includes(b)))return false;
    if(NOISE_RX.some(p=>p.test(kw)))return false;
    return true;
  });
}
...
function buildBalancedList(kws,target){
  ...
  if(!cats.length)return kws.slice(0,target);
  const total=kws.length;
  const selected=[];
  cats.forEach(cat=>{
    const quota=Math.max(1,Math.round(target*(groups[cat].length/total)));
    selected.push(...[...groups[cat]].sort((a,b)=>b.volume-a.volume).slice(0,quota));
  });
...
// CORRECT ORDER:
// 0=Projet, 1=Catégories, 2=Mots-clés, 3=Volumes, 4=Nettoyage, 5=Catégorisation, 6=Export
...
function StepBar({step}){
  return <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>{STEPS.map((l,i)=><div key={i} style={{padding:"4px 11px",borderRadius:4,border:`1px solid ${i===step?G.accent:i<step?G.green:G.border}`,color:i===step?G.accent:i<step?G.green:G.muted,fontSize:11}}>{i<step?"✓":i+1}. {l}</div>)}</div>;
}
...
export default function App(){
  ...
  // Step 2 — keyword collection
  const [kwSub,setKwSub]=useState(0);
  const [competitors,setCompetitors]=useState([]);
  const [sourceKws,setSourceKws]=useState([]);

  // Step 3 — volumes (comes BEFORE cleaning)
  const [rawWithVol,setRawWithVol]=useState([]); // all sources merged with volumes, vol>0 only

  // Step 4 — cleaning (with volumes available)
  ...
  async function extractCategories(){
    ...
    try{
      const resp=await callClaude(
        `Expert SEO. Extrais les sous-catégories produits génériques du menu. Règles : types de produits génériques uniquement · pas de marques SEB · pas de gammes/collections · pas de catégories générales · "A & B" → deux entrées · pratique → type produit. JSON : {"categories":["cat1",...]}`,
        `Site:${config.siteUrl}|Pays:${config.country}|Langue:${config.language}`,catImages
      );
      const{categories:cats}=parseJSON(resp);
  ...
  async function runEnrichment(){
    ...
    try{
      const resp=await callClaude(
        `Assistant SEO multilingue. Génère 10 requêtes SEO génériques, evergreen, hors-marque par catégorie. Langue locale uniquement. Formulations : meilleur, avis, acheter, pas cher, entretien, quel choisir… Jamais de marques, années, promos. JSON : {"enriched":[{"category":"...","keywords":["kw1",...10]},...]}`,
        `Pays:${config.country}|Langue:${config.language}\nCatégories:\n${categories.join("\n")}`
      );
      ...
      setEnrichedKws(enriched.flatMap(e=>e.keywords.map(k=>({keyword:k,volume:0}))));
      addLog(`✅ ${enriched.flatMap(e=>e.keywords).length} mots-clés sémantiques générés`);
    }catch(e){setErr("Erreur: "+e.message);}
    setLoading(false);
  }

  // ── Step 2: Competitors + sources ──────────────────────────────────────────
  async function findCompetitors(){
    setLoading(true);setErr("");setLoadMsg("Identification concurrents…");
    try{
      const resp=await callClaude(
        `Expert marché équipement domestique. Identifie 5 concurrents en ligne d'une marque Groupe SEB. Pour chacun : nom, domaine exact (sans https://), explication 1 phrase. JSON : {"competitors":[{"name":"...","url":"...","reason":"..."},...]}`,
        `Marque:${config.brand}|Pays:${config.country}|Langue:${config.language}|Site:${config.siteUrl}`
      );
      const{competitors:comps}=parseJSON(resp);
      setCompetitors(comps);addLog(`✅ ${comps.length} concurrents identifiés`);
    }catch(e){setErr("Erreur: "+e.message);}
    setLoading(false);
  }

  async function replaceComp(idx){
    setLoading(true);setErr("");setLoadMsg(`Remplacement de ${competitors[idx].name}…`);
    const snap=[...competitors];
    try{
      const resp=await callClaude(
        `Propose UN seul concurrent alternatif pertinent, différent des autres. JSON : {"name":"...","url":"...","reason":"..."}`,
        `Marque:${config.brand}|Pays:${config.country}\nÀ remplacer:${snap[idx].name}\nDéjà présents:${snap.filter((_,j)=>j!==idx).map(x=>x.name).join(", ")}`
      );
      const alt=parseJSON(resp);
      setCompetitors(prev=>prev.map((x,j)=>j===idx?alt:x));
      addLog(`🔄 ${snap[idx].name} → ${alt.name}`);
    }catch(e){setErr("Erreur: "+e.message);}
    setLoading(false);
  }
  ...
  async function importSourceFiles(files){
    ...
    for(const f of files){
      let rows=[];
      try{rows=await parseFile(f);}catch(e){addLog(`⚠ ${f.name}: ${e.message}`);}
      all.push(...rows.map(r=>({keyword:r.keyword,volume:0})));
      addLog(`📄 ${f.name} → ${rows.length} mots-clés`);
    }
    setSourceKws(prev=>dedup([...prev,...all]));
  }

  // ── Step 3: Import volumes — BEFORE cleaning ───────────────────────────────
  async function importVolumeFile(files){
    const f=files[0];if(!f)return;
    let rows=[];
    try{rows=await parseFile(f);}catch(e){setErr(`Erreur ${f.name}: ${e.message}`);return;}
    const withVol=rows.filter(r=>r.volume>0);
    if(!withVol.length){setErr(`Aucun volume trouvé dans "${f.name}".`);return;}
    // Match volumes to source keywords
    const volMap=new Map(withVol.map(r=>[r.keyword.toLowerCase().trim(),r.volume]));
    const sources=allSourceKws();
    const merged=sources
      .map(k=>({...k,volume:volMap.get(k.keyword.toLowerCase().trim())||0}))
      .filter(k=>k.volume>0) // keep only keywords with a real volume
      .sort((a,b)=>b.volume-a.volume);
    const dropped=sources.length-merged.length;
    setRawWithVol(merged);
    setTargetCount(Math.min(500,merged.length));
    addLog(`📊 ${f.name} → ${merged.length} mots-clés avec volumes (${dropped} sans volume écartés)`);
    setErr("");
  }

  // ── Step 4: Cleaning WITH volumes — dedup by similarity + volume ───────────
  function runRuleBasedCleaning(){
    const before=rawWithVol.length;
    const cleaned=ruleBasedClean(rawWithVol); // already sorted by volume desc
  ...
  async function runClaudeDedup(){
    ...
    const BATCH=80; // small batches to avoid Cloudflare 524 timeout
    const total=Math.ceil(cleanedKws.length/BATCH);
    ...
    try{
      for(let b=0;b<total;b++){
        ...
        const resp=await callClaude(
          `Tu analyses une liste de mots-clés SEO triés par volume décroissant.
Identifie les quasi-doublons : même intention + très forte similarité (pluriel, accent, faute, ordre des mots).
RÈGLE : garde toujours le mot-clé avec le PLUS GRAND volume. Liste uniquement ceux à supprimer.
Ne supprime PAS des mots-clés avec des sens différents même s'ils se ressemblent.
JSON uniquement : {"remove":["kw1","kw2",...]} — tableau vide si aucun doublon.`,
      ...
      setCleanedKws(deduped);
      setCleanStats(s=>({...s,afterDedup:deduped.length,dedupRemoved:toRemove.size}));
      addLog(`✅ Dédoublonnage : ${deduped.length} conservés (${toRemove.size} quasi-doublons supprimés)`);
    }catch(e){setErr("Erreur: "+e.message);}
    setLoading(false);
  }
  ...
  async function runClaudeSemanticPass(){
    if(!cleanedKws.length)return;
    setLoading(true);setErr("");
    const BATCH=80; // small batches to avoid Cloudflare timeout
    const total=Math.ceil(cleanedKws.length/BATCH);
    const kept=[];
    try{
      for(let b=0;b<total;b++){
        ...
        const batch=cleanedKws.slice(b*BATCH,(b+1)*BATCH);
        const resp=await callClaude(
          `Expert SEO. Supprime uniquement les mots-clés clairement hors-périmètre des catégories fournies. Conserve tout ce qui est pertinent.
JSON : {"kept":["kw1","kw2",...]}`,
      ...
      setCleanedKws(kept);
      setCleanStats(s=>({...s,afterSemantic:kept.length}));
      addLog(`✅ Affinage sémantique : ${kept.length} conservés`);
    }catch(e){setErr("Erreur: "+e.message);}
    setLoading(false);
  }

  // ── Step 5: Categorization in batches ─────────────────────────────────────
  async function runCategorization(){
    setLoading(true);setErr("");
    const kws=cleanedKws.filter(k=>k.volume>0);
    const BATCH=80; // small batches to avoid Cloudflare timeout
    const allResults=[];
    const total=Math.ceil(kws.length/BATCH);
    try{
      for(let b=0;b<total;b++){
...
100% assignés, jamais Uncategorized, ne pas modifier le texte.
JSON : {"results":[{"keyword":"...","category":"...","sebCategory":"..."},...]}`,
      ...
      setCategorizedKws(final);
      setTargetCount(Math.min(targetCount,final.length));
      addLog(`✅ Catégorisation : ${final.length} mots-clés (${total} batches)`);
      nav(6);
      ...
      <div style={{background:"#0a0d14",borderBottom:`1px solid ${G.border}`,padding:"14px 24px",display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
        ...
        <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:12}}>
          ...
          {step>0&&<button onClick={()=>{
            if(!window.confirm("Recommencer un nouveau projet ? Toutes les données en cours seront effacées."))return;
            setStep(0);setErr("");setLog([]);
            setConfig({brand:"",country:"",language:"",siteUrl:""});
            setCatImages([]);setCategories([]);setCatManual("");
            setEnrichedKws([]);setEnrichPreview("");setEnrichValidated(false);
            setKwSub(0);setCompetitors([]);setSourceKws([]);
            setRawWithVol([]);setCleanedKws([]);setCleanStats(null);
            setCategorizedKws([]);setTargetCount(500);
          }} style={{background:"none",border:`1px solid ${G.red}44`,color:G.red+"cc",padding:"5px 12px",borderRadius:6,fontFamily:"inherit",fontSize:11,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}>
            🔄 Nouveau projet
          </button>}
        </div>
      </div>
      ...
      <div style={{maxWidth:860,margin:"0 auto",padding:"28px 20px"}}>
        ...
        {step===1&&<Card>
          ...
          {categories.length>0&&<>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:20}}>
              {categories.map((c,i)=><span key={i} style={{display:"inline-flex",alignItems:"center",gap:4,background:G.accent+"18",border:`1px solid ${G.accent}33`,color:G.accent,padding:"3px 10px",borderRadius:20,fontSize:12}}>
                {c}<button onClick={()=>setCategories(p=>p.filter((_,j)=>j!==i))} style={{background:"none",border:"none",color:G.muted,cursor:"pointer",padding:0,fontSize:11}}>×</button>
              </span>)}
            </div>
            ...
            <div style={{borderTop:`1px solid ${G.border}`,paddingTop:18}}>
              ...
              <Info color={G.green}>Claude génère des requêtes evergreen hors-marque (meilleur X, acheter X, avis X…) qui seront ajoutées à ta liste avant les volumes.</Info>
              {!loading&&enrichedKws.length===0&&<Btn onClick={runEnrichment} color={G.green}>🤖 Générer</Btn>}
              {enrichPreview&&!enrichValidated&&<>
                <div style={{background:G.faint,borderRadius:8,padding:12,fontSize:12,color:G.sub,whiteSpace:"pre-wrap",lineHeight:1.7,marginTop:12,marginBottom:10}}>{enrichPreview}</div>
                <div style={{display:"flex",gap:10}}>
                  <SBtn onClick={runEnrichment} color={G.yellow}>🔄 Regénérer</SBtn>
                  <Btn onClick={()=>{setEnrichValidated(true);addLog(`✅ ${enrichedKws.length} mots-clés sémantiques validés`);}} color={G.green}>✅ Valider</Btn>
                </div>
              </>}
        ...
        {step===2&&<>
          {kwSub===0&&<Card>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
              <div onClick={()=>setKwSub(1)} style={{background:G.faint,border:`1px solid ${G.border}`,borderRadius:10,padding:20,cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.borderColor=G.accent} onMouseLeave={e=>e.currentTarget.style.borderColor=G.border}>
                ...
                <div style={{fontSize:14,color:G.text,fontWeight:700,marginBottom:5}}>Je pars de zéro</div>
              ...
              <div onClick={()=>setKwSub(3)} style={{background:G.faint,border:`1px solid ${G.border}`,borderRadius:10,padding:20,cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.borderColor=G.accent} onMouseLeave={e=>e.currentTarget.style.borderColor=G.border}>
                <div style={{fontSize:26,marginBottom:8}}>📂</div>
                <div style={{fontSize:14,color:G.text,fontWeight:700,marginBottom:5}}>J'ai déjà des fichiers</div>
          ...
          {kwSub===1&&<Card>
            ...
            <Sub c={`5 principaux concurrents de ${config.brand} sur le marché ${config.country}.`}/>
            {loading?<Spin msg={loadMsg}/>:<>
              ...
              {competitors.length>0&&<>
                ...
                {competitors.map((c,i)=>{
                  const idx=i;
                  return <div key={i} style={{background:G.faint,border:`1px solid ${G.border}`,borderRadius:8,padding:"12px 14px",marginBottom:8}}>
                    ...
                    <div style={{display:"flex",gap:8}}>
                      <SBtn onClick={()=>replaceComp(idx)} color={G.yellow}>🔄 Non pertinent — remplacer</SBtn>
                      <SBtn onClick={()=>setCompetitors(prev=>prev.filter((_,j)=>j!==idx))} color={G.red}>🗑 Supprimer</SBtn>
                    </div>
                  </div>;
                })}
                <AddCompForm onAdd={c=>setCompetitors(prev=>[...prev,c])}/>
                <div style={{marginTop:16,display:"flex",gap:10,justifyContent:"space-between"}}>
                  <Btn secondary onClick={findCompetitors} style={{fontSize:12}}>🔄 Regénérer</Btn>
                  <Btn onClick={()=>setKwSub(2)}>Valider → Liens SEMrush</Btn>
                </div>
              </>}
            </>}
          </Card>}
          ...
          {kwSub===2&&<Card>
            ...
            {competitors.map((c,i)=>{
              ...
              const db=getDb();
              const f50comp=encodeURIComponent(JSON.stringify({search:null,volume:null,kd:null,intent:null,position:{value:"0-50",type:"competitors"},advanced:{}}));
              ...
              return <div key={i} style={{background:G.faint,border:`1px solid ${G.border}`,borderRadius:10,padding:16,marginBottom:12}}>
                <div style={{marginBottom:10}}>
                  <span style={{fontSize:13,color:G.text,fontWeight:700}}>{config.brand}</span>
                  <span style={{color:G.muted}}> vs </span>
                  <span style={{fontSize:13,color:G.accent,fontWeight:700}}>{c.name}</span>
                  <div style={{fontSize:11,color:G.muted,marginTop:2}}>{config.siteUrl} ↔ {comp}</div>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
                  {[
                    {label:"Shared",rt:"common",color:G.accent,desc:"Mots-clés en commun",f:""},
                    {label:"Missing",rt:"missing",color:G.yellow,desc:"Top 50 concurrent, absent chez vous",f:`&filter=${f50comp}`},
                    {label:"Untapped",rt:"untapped",color:G.green,desc:"Top 50 concurrent, opportunités",f:`&filter=${f50comp}`},
                  ].map(({label,rt,color,desc,f})=>(
          ...
          {kwSub===3&&<Card>
            {competitors.length>0&&<div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20}}>
              <Btn secondary onClick={()=>setKwSub(2)} style={{padding:"5px 12px",fontSize:12}}>← Retour SEMrush</Btn>
              <div style={{flex:1,height:1,background:G.border}}/><span style={{fontSize:11,color:G.muted}}>3/3 — Import sources</span>
            </div>}
            <H2 c="📂 Import des sources de mots-clés"/>
            <Info>Tous tes fichiers sources ici : SEMrush, anciennes listes, SEA, retailers…<br/><strong style={{color:G.text}}>Uniquement les keywords extraits — les volumes viennent à l'étape suivante.</strong></Info>
            ...
            {(sourceKws.length>0||enrichedKws.length>0)&&<div style={{marginTop:10,background:G.faint,borderRadius:8,padding:"10px 14px",fontSize:12,color:G.sub}}>
              <strong style={{color:G.text}}>Total collecté :</strong> {allSourceKws().length} mots-clés{enrichedKws.length>0&&<span style={{color:G.green}}> (dont {enrichedKws.length} sémantiques)</span>}
            </div>}
        ...
        {/* ── 3: VOLUMES (avant nettoyage) ───────────────────────────────── */}
        {step===3&&<Card>
          <H2 c="📊 Import des volumes"/>
          <Sub c="Les volumes arrivent AVANT le nettoyage — c'est eux qui guident le dédoublonnage et la sélection."/>
          <Info color={G.green}>
            <strong style={{color:G.text}}>Flux :</strong><br/>
            1. Télécharge d'abord la liste pour KWP si besoin → <SBtn onClick={()=>dl(allSourceKws().map(k=>k.keyword).join("\n"),`${config.brand}_${config.country}_pour_KWP.txt`)} color={G.accent}>📥 Télécharger pour Keyword Planner ({allSourceKws().length} mots-clés)</SBtn><br/>
            2. Dans <a href="https://ads.google.com/aw/keywordplanner/ideas/existing" target="_blank" rel="noreferrer" style={{color:G.accent}}>Google Keyword Planner</a> → Get search volume → sélectionne le pays → exporte CSV<br/>
            3. Glisse ce fichier ici. Les mots-clés sans volume seront écartés automatiquement.
          </Info>
          ...
          <Drop label="Fichier de volumes" note="Keyword Planner CSV · SEMrush XLSX · tout fichier keyword + volume" accept=".csv,.tsv,.txt,.xlsx,.xls" multiple={false} onFiles={importVolumeFile}>
            {rawWithVol.length>0&&<div style={{marginTop:8}}>
              <span style={{fontSize:13,color:G.green,fontWeight:600}}>✅ {rawWithVol.length} mots-clés avec volumes</span>
              <span style={{fontSize:11,color:G.muted,marginLeft:10}}>Volume max : {rawWithVol.reduce((m,r)=>r.volume>m?r.volume:m,0).toLocaleString()}</span>
            </div>}
          </Drop>
          ...
          {rawWithVol.length>0&&<>
            ...
            <div style={{marginTop:12,maxHeight:160,overflowY:"auto",fontSize:12}}>
              {rawWithVol.slice(0,10).map((k,i)=>(
                <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"3px 0",borderBottom:`1px solid ${G.faint}`}}>
                  <span style={{color:G.sub}}>{k.keyword}</span>
                  <span style={{color:G.accent}}>{k.volume.toLocaleString()}</span>
                </div>
              ))}
        ...
        {/* ── 4: NETTOYAGE (avec volumes) ────────────────────────────────── */}
        {step===4&&<Card>
          <H2 c="🧹 Nettoyage + dédoublonnage intelligent"/>
          <Sub c="Maintenant que les volumes sont là, on nettoie intelligemment : deux mots-clés quasi-identiques → on garde celui avec le plus grand volume."/>

          <Tag n="Passe 1">Nettoyage par règles — instantané</Tag>
          <Info>Supprime automatiquement : marques connues · recettes · années · promos. Résultat immédiat, sans limite de taille.</Info>
          ...
          {!cleanStats
            ?<Btn onClick={()=>{runRuleBasedCleaning();addLog("🧹 Nettoyage règles lancé");}}>⚡ Lancer le nettoyage (instantané)</Btn>
            :<div style={{background:G.green+"14",border:`1px solid ${G.green}44`,borderRadius:8,padding:"12px 14px",fontSize:12,color:G.green,marginBottom:12}}>
              ...
              {cleanStats.afterDedup!==undefined&&` · Dédup : ${cleanStats.afterDedup} (${cleanStats.dedupRemoved} quasi-doublons supprimés)`}
              {cleanStats.afterSemantic!==undefined&&` · Sémantique : ${cleanStats.afterSemantic}`}
            </div>
          }
          ...
          {cleanStats&&<>
            ...
            <div style={{marginBottom:16}}>
              ...
              <div style={{fontSize:11,color:G.muted,marginTop:4}}>Clique ✕ pour supprimer manuellement un mot-clé</div>
            </div>
            ...
            <div style={{borderTop:`1px solid ${G.border}`,paddingTop:18,display:"flex",flexDirection:"column",gap:12}}>
              ...
              <Info color={G.yellow}>Claude repère les quasi-doublons (pluriel, accent, faute, ordre des mots) et garde toujours celui avec le plus grand volume. Batches de 80 mots-clés.</Info>
            ...
            <div style={{borderTop:`1px solid ${G.border}`,paddingTop:18,marginTop:4}}>
              ...
              <Info color={G.muted}>Claude supprime les mots-clés hors-périmètre que les règles n'ont pas capturés. Batches de 80 mots-clés.</Info>
        ...
        {step===5&&<Card>
          <H2 c="🏷 Catégorisation"/>
          <Sub c="Catégorie produit + grande catégorie SEB. Traitement par lots de 150."/>
          ...
          <Stat value={cleanedKws.length} label="Mots-clés à catégoriser (triés par volume)"/>
          {loading?<Spin msg={loadMsg}/>:(
            <div style={{display:"flex",justifyContent:"space-between",marginTop:20}}>
              <Btn secondary onClick={()=>nav(4)}>← Retour</Btn>
              <Btn onClick={runCategorization}>🤖 Catégoriser</Btn>
            </div>
          )}
        </Card>}
        ...
        {step===6&&<Card>
          ...
          <Sub c="Choisis ta cible — l'app répartit proportionnellement par catégorie en conservant le ratio naturel et les meilleurs volumes."/>
            ...
            return <>
              <div style={{background:G.faint,border:`1px solid ${G.border}`,borderRadius:10,padding:20,marginBottom:20}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:12}}>
                  <input type="range" min={50} max={Math.min(2000,categorizedKws.length)} step={50} value={targetCount}
                ...
                <div style={{fontSize:12,color:G.sub,marginBottom:10}}>
                  <strong style={{color:G.text}}>{Object.keys(groups).length} catégories</strong> · ratio naturel conservé · meilleurs volumes prioritaires
                </div>
                ...
                <Btn onClick={()=>{
                  try{dlCSV([["Main SEB Category","Keyword","Avg. Monthly Volume","Product Category"],...balanced.map(k=>[k.sebCategory||"",k.keyword,k.volume,k.category||""])],`${config.brand}_${config.country}_keywords_${targetCount}.csv`);setTimeout(()=>addLog(`📥 ${balanced.length} mots-clés exportés`),0);}
                  catch(e){setErr("Erreur export: "+e.message);}
                }}>📥 Télécharger — {targetCount} mots-clés équilibrés</Btn>
              </div>

              <div style={{maxHeight:200,overflowY:"auto",fontSize:12}}>
                <div style={{display:"grid",gridTemplateColumns:"140px 1fr 70px 130px",position:"sticky",top:0,background:G.card,paddingBottom:5}}>
                  {["SEB Cat.","Mot-clé","Volume","Catégorie"].map(h=><div key={h} style={{color:G.muted,fontSize:10,textTransform:"uppercase",letterSpacing:".06em",padding:"4px 6px"}}>{h}</div>)}
                </div>
                {balanced.slice(0,20).map((k,i)=>(
                  <div key={i} style={{display:"grid",gridTemplateColumns:"140px 1fr 70px 130px",background:i%2===0?G.faint:"transparent",borderRadius:4}}>
